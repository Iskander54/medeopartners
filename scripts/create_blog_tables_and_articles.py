#!/usr/bin/env python3
"""
Script SQL direct pour créer les tables du blog et insérer les articles
"""
...
print("📦 Création des tables du blog...")

# Créer la table blog_category
cursor.execute("""
...
# Créer la table de liaison article_tags
cursor.execute("""
...
categories = [
    ("Fiscalité", "fiscalite", "Articles sur la fiscalité, la TVA et les obligations fiscales"),
    ("Création d'entreprise", "creation-entreprise", "Conseils et guides pour créer votre entreprise")
]
...
for name, slug, desc in categories:
    ...
    if existing:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (name, slug, desc, f"Découvrez nos articles sur {name}", datetime.now()))
...
N'hésitez pas à consulter un cabinet d'expertise comptable pour bénéficier d'un accompagnement personnalisé et sécuriser votre gestion de la TVA. Un professionnel pourra analyser votre situation spécifique et vous proposer les solutions les plus adaptées à votre activité.""",
        ...
        "secondary_keywords": '["déclaration TVA", "régime TVA", "TVA déductible", "TVA collectée", "franchise en base TVA", "régime réel simplifié", "déclaration CA3", "expert comptable TVA"]',
...
N'hésitez pas à consulter un cabinet d'expertise comptable dès la création de votre entreprise. Un accompagnement professionnel dès le départ vous permettra d'éviter les erreurs coûteuses et de vous concentrer sur le développement de votre activité.""",
        ...
        "secondary_keywords": '["erreurs fiscales création entreprise", "première année entreprise", "comptabilité création entreprise", "expert comptable création entreprise", "obligations fiscales entreprise", "régime fiscal entreprise", "démarrage entreprise", "conseil création entreprise"]',
...
print("📝 Insertion des articles SEO...")
now = datetime.now()
...
for article_data in ARTICLES:
    ...
    category_id = category_ids[article_data["category_slug"]]
    ...
    print(f"   - Catégorie : {article_data['category_slug']}")
...
print("=" * 60)
print("✨ Les articles sont maintenant disponibles sur votre blog !")
print(f"   URL : https://www.medeo-partners.com/fr/blog/")
print()

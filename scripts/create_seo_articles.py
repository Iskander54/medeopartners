#!/usr/bin/env python3
"""
Script pour créer des articles SEO optimisés pour le blog Medeo Partners
"""
...
def get_or_create_category(slug, name, description=None):
    ...
    if not category:
        category = BlogCategory(
            name=name,
            slug=slug,
            description=description or f"Articles sur {name}",
...
def get_or_create_tag(name):
    ...
    if not tag:
        tag = BlogTag(
            name=name,
            slug=slug,
            description=f"Articles taggés {name}"
        )
        db.session.add(tag)
        db.session.commit()
...
def create_articles():
    """Crée les articles SEO dans la base de données"""
    ...
    with app.app_context():
        ...
        for slug, (name, desc) in categories_map.items():
            get_or_create_category(slug, name, desc)

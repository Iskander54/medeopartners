#!/usr/bin/env python3
"""
Script SQL direct pour insérer les articles dans la base de données
"""
...
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'medeo', 'site.db')
...
def insert_articles():
    """Insère les articles dans la base de données SQLite"""
    ...
    if not cursor.fetchone():
        print("❌ Les tables du blog n'existent pas.")
        print("💡 Veuillez d'abord lancer l'application Flask pour créer les tables.")
        conn.close()
        return
    
    now = datetime.now()
    
    # Créer les catégories et articles
    for article_data in ARTICLES_DATA:
        # Créer ou récupérer la catégorie
        cursor.execute("SELECT id FROM blog_category WHERE slug = ?", (article_data["category_slug"],))
        ...
        if not category:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (
                article_data["category_name"],
                article_data["category_slug"],
                f"Articles sur {article_data['category_name']}",
                f"Découvrez nos articles sur {article_data['category_name']}",
                now
            ))
            category_id = cursor.lastrowid
            print(f"✅ Catégorie créée : {article_data['category_name']}")
        else:
        ...
        print(f"   - Catégorie : {article_data['category_name']}")
